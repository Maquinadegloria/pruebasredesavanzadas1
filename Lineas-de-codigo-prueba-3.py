...
class AdministradorRedes:
    ...
    def administrar_dispositivos(self):
        ...
        while True:
            ...
            if opcion == "1":
                self.agregar_dispositivo(nombre_campus)
            elif opcion == "2":
    ...
    def agregar_dispositivo(self, nombre_campus):
        ...
        capa = input("Ingrese la capa jerárquica del dispositivo: ")
        interfaces = input("Ingrese las interfaces del dispositivo, separadas por comas: ").split(",")
        ips_masks = {}
        for interface in interfaces:
            while True:
                ip = input(f"Ingrese la dirección IP para la interfaz {interface}: ")
                if es_direccion_ipv4(ip) or es_direccion_ipv6(ip):
                    break
                else:
                    print("Dirección IP no válida. Inténtelo de nuevo.")
            mask = input(f"Ingrese la máscara de red para la interfaz {interface}: ")
            ips_masks[interface] = (ip, mask)
        vlans = {}
        while True:
            vlan = input("Ingrese el nombre de la VLAN (o presione Enter para terminar): ")
            if not vlan:
                break
            numero = input(f"Ingrese el número de la VLAN {vlan}: ")
            vlans[vlan] = numero
    ...
    def modificar_dispositivo(self, nombre_campus):
        ...
        dispositivo = next((d for d in self.campus[nombre_campus].dispositivos if d.nombre == nombre_dispositivo), None)
        if not dispositivo:
            input("El dispositivo especificado no existe. Presione Enter para continuar.")
            return

        dispositivo.modelo = input(f"Ingrese el nuevo modelo del dispositivo (actual: {dispositivo.modelo}): ") or dispositivo.modelo
        dispositivo.capa = input(f"Ingrese la nueva capa jerárquica del dispositivo (actual: {dispositivo.capa}): ") or dispositivo.capa
        nuevas_interfaces = input("Ingrese las nuevas interfaces del dispositivo, separadas por comas (o presione Enter para mantener las actuales): ")
        if nuevas_interfaces:
            dispositivo.interfaces = nuevas_interfaces.split(",")
            ...
            for interface in dispositivo.interfaces:
                while True:
                    ...
                    if es_direccion_ipv4(ip) or es_direccion_ipv6(ip):
                        ...
                        print("Dirección IP no válida. Inténtelo de nuevo.")
                mask = input(f"Ingrese la máscara de red para la interfaz {interface}: ")
                dispositivo.ips_masks[interface] = (ip, mask)
        nuevas_vlans = input("Ingrese las nuevas VLANs del dispositivo, separadas por comas (o presione Enter para mantener las actuales): ")
        if nuevas_vlans:
            dispositivo.vlans = {}
            for vlan in nuevas_vlans.split(","):
                numero = input(f"Ingrese el número de la VLAN {vlan}: ")
                dispositivo.vlans[vlan] = numero
        dispositivo.servicios = input(f"Ingrese los nuevos servicios del dispositivo, separados por comas (actuales: {', '.join(dispositivo.servicios)}): ").split(",") or dispositivo.servicios
        input("Dispositivo modificado. Presione Enter para continuar.")

    def ver_dispositivo(self, nombre_campus):
        """Muestra la información de un dispositivo en un campus."""
    ...
    def ver_campus(self):
        """Muestra la información de todos los campus y sus dispositivos."""
